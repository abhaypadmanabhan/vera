/*
 * Vendored from ElevenLabs' shadcn registry (https://ui.elevenlabs.io/r/orb.json)
 * via `npx shadcn add`. Kept as close to upstream as possible so it can be
 * refreshed; the only deliberate change is `isDarkSurface()` below.
 *
 * React Compiler's rules are switched off for this file, not waived across the
 * repo. A three.js scene legitimately mutates the texture and the uniform
 * objects it hands to the GPU and reads refs while building them — rewriting
 * that to satisfy the compiler would mean rewriting someone else's shader
 * plumbing, and every upstream refresh would have to be rewritten again.
 * Nothing in Vera's own code is exempt.
 */
/* eslint-disable react-hooks/immutability, react-hooks/purity, react-hooks/refs, react-hooks/preserve-manual-memoization */
"use client"
...
function Scene({
  ...
  useEffect(() => {
    const apply = () => {
      ...
      circleRef.current.material.uniforms.uInverted.value = isDarkSurface() ? 1 : 0
    }
...
/**
 * Local change to the registry component: ElevenLabs detect dark mode from a
 * `.dark` class on <html>. Vera drives dark mode from `prefers-color-scheme`
 * with a `data-theme` override (app/globals.css), so the shader would never
 * invert and would paint a white disc on the dark surface. The `.dark` class is
 * still honoured so an upstream refresh of this file loses nothing.
 */
